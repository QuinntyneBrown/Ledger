...
secured.MapGet("/preferences", async (LedgerDbContext db, IUserContext user, CancellationToken ct) => (await db.Preferences.AsNoTracking().SingleAsync(x => x.UserId == user.UserId, ct)).ToDto()); secured.MapPatch("/preferences", (ChangePreferencesCommand c, ISender s, CancellationToken ct) => s.Send(c, ct)); secured.MapPut("/reminders", (ScheduleReminderCommand c, ISender s, CancellationToken ct) => s.Send(c, ct));
secured.MapPost("/push-subscriptions", async (PushSubscriptionBody b, LedgerDbContext db, IUserContext user, CancellationToken ct) => { var existing = await db.PushSubscriptions.SingleOrDefaultAsync(x => x.UserId == user.UserId && x.Endpoint == b.Endpoint, ct); if (existing is null) db.PushSubscriptions.Add(new PushSubscription { UserId = user.UserId, Endpoint = b.Endpoint, P256dh = b.P256dh, Auth = b.Auth }); else { existing.P256dh = b.P256dh; existing.Auth = b.Auth; } await db.SaveChangesAsync(ct); return Results.NoContent(); });
