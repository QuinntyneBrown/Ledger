...
public sealed class LedgerDbContext(DbContextOptions<LedgerDbContext> options) : DbContext(options), ILedgerDbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<WeighIn>().Property(x => x.WeightKg).HasPrecision(6, 1);
        b.Entity<OnboardingDraft>().Property(x => x.CurrentWeightKg).HasPrecision(6, 1);
        b.Entity<OnboardingDraft>().Property(x => x.GoalWeightKg).HasPrecision(6, 1);
        b.Entity<User>().Property(x => x.HeightCm).HasPrecision(5, 1);
        b.Entity<Goal>().HasIndex(x => x.UserId).IsUnique();
        b.Entity<Goal>().Property(x => x.StartWeightKg).HasPrecision(6, 1);
        b.Entity<Goal>().Property(x => x.GoalWeightKg).HasPrecision(6, 1);
