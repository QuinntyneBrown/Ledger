...
test('marketing site presents the product and links into the application',async({page})=>{await page.goto('http://127.0.0.1:4300');await expect(page.getByRole('heading',{level:1})).toContainText('Weight');await expect(page.getByRole('link',{name:/start your ledger/i})).toHaveAttribute('href','http://localhost:4200/register');await expect(page.getByRole('link',{name:'Sign in'})).toHaveAttribute('href','http://localhost:4200/sign-in');await page.goto('http://127.0.0.1:4300/privacy');await expect(page.getByRole('heading',{level:1})).toHaveText('Privacy Policy');});
